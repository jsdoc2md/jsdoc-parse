...
exports.publish = function(data) {
    ...
    var json = a.where(data().get(), query).map(function(record){
        ...
        */
        if (record.tags){
            ...
            var todo = a.arrayify(record.todo);
            delete record.todo;
            var done = a.extract(record.tags, { title: "done" });
            var todoList = []
                .concat(todo.map(function(task){
                    return { done: false, task: task };
                }))
                .concat(done.map(function(task){
                    return { done: true, task: task.value };
                }));

            if (todoList.length){
                record.todoList = todoList;
            }

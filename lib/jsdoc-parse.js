...
@param {module:jsdoc-parse~parseOptions} - parse options
@returns {external:Transform}
@example
```js
...
function applyOptions(data, options){
    ...
    if(options.stats) {
        var stats = {};
        stats.class = a(data).where({ kind: "class" }).pluck("longname").val();
        stats.module = a(data).where({ kind: "module" }).pluck("longname").val();
        stats.module = a(data).where({ kind: "module" }).pluck("longname").val();
...
/**
@typedef {object} module:jsdoc-parse~parseOptions
@property {boolean} [stats=false] - Return stats about the doclets parsed
@property {boolean} [private=false] - include @private members in the output
@property {boolean} [html=false] - if set, you can parse jsdoc from html files
@property {Array} [sort-by=\[ "scope", "category", "kind", "order" \]] - sort the output
*/
